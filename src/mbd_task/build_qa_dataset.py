"""
SQuAD-style QA dataset builder for the Citilink minutes project.
...
def load_multiple_citilink(files):
    """
    Loads and merges multiple Citilink metadata JSON files.
...
def main():
    """
    Builds train/val/test SQuAD v2 datasets from Citilink metadata files.
    """
    ...
    ap.add_argument("--splits_json", type=Path,
                    default=Path("src/metadata_identification/Dataset/splits.json"),
    ...
    if args.lang == "pt":
        ...
        input_jsons = [
            "MiNER/data/dataset_metadata_pt/M01_pt.json",
            "MiNER/data/dataset_metadata_pt/M02_pt.json",
            "MiNER/data/dataset_metadata_pt/M03_pt.json",
            "MiNER/data/dataset_metadata_pt/M04_pt.json",
            "MiNER/data/dataset_metadata_pt/M05_pt.json",
            "MiNER/data/dataset_metadata_pt/M06_pt.json",
        ]
        output_dir = Path("data/qa_squad/global_split")
    else:
        ...
        input_jsons = [
            "src/metadata_identification/Dataset/citilink_metadata_en/alandroal_citilink_en.json",
            "src/metadata_identification/Dataset/citilink_metadata_en/campomaior_citilink_en.json",
            "src/metadata_identification/Dataset/citilink_metadata_en/covilha_citilink_en.json",
            "src/metadata_identification/Dataset/citilink_metadata_en/fundao_citilink_en.json",
            "src/metadata_identification/Dataset/citilink_metadata_en/guimaraes_citilink_en.json",
            "src/metadata_identification/Dataset/citilink_metadata_en/porto_citilink_en.json",
        ]
        output_dir = Path("data/qa_squad_en/global_split")

    print(f"🔤 Selected language: {args.lang}")
    print(f"📂 Output directory: {output_dir}")

    # Load documents and splits
    citilink_docs = load_multiple_citilink(input_jsons)
    ...
    for municipality, docs in citilink_docs.items():
        for doc_id, doc in docs.items():
