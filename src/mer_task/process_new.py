    ...
    for ent in combined_spans:
        ...
        if ent["source"] == "municipio":
            ...
            if original_span not in nome_map:
                nome_map[original_span] = faker.name()
...
def load_existing_jsonl(path):
    """Load an existing JSONL file and return (examples, ids)."""
...
def main():
    ...
    parser.add_argument("--input_dir", type=str, default="data/dataset_metadata_pt",
                        help="Path to directory containing JSON files") 
    parser.add_argument("--output_dir", type=str, default="data/metadata_paper_pt",
    ...
    # Load dataset splits
    with open("data/split/splits.json", "r", encoding="utf-8") as f:
        splits = json.load(f)
    ...
    # Accumulators for combined datasets
    combined_train = []
    combined_val = []
    combined_test = []

    # ------------------------------------------------------------------
    # Process each municipality with MERGE BEHAVIOUR 
    # ------------------------------------------------------------------
    for municipality_name, data in municipality_data.items():
        ...
        existing_train, existing_train_ids = load_existing_jsonl(
            os.path.join(train_dir, f"{municipality_clean}_train.jsonl")
        )
        existing_val, existing_val_ids = load_existing_jsonl(
            os.path.join(val_dir, f"{municipality_clean}_val.jsonl")
        )
        existing_test, existing_test_ids = load_existing_jsonl(
            os.path.join(test_dir, f"{municipality_clean}_test.jsonl")
        )

        # Combined set of existing ids
        existing_ids = existing_train_ids | existing_val_ids | existing_test_ids

        # -------- Process NEW dataset entries --------
        new_train, new_val, new_test = process_dataset_records(
            [data], train_files, val_files, test_files, municipality_name
        )

        # -------- Filter only NEW atás not present in existing --------
        new_train_filtered = [ex for ex in new_train if ex["ata_id"] not in existing_ids]
        new_val_filtered = [ex for ex in new_val if ex["ata_id"] not in existing_ids]
        new_test_filtered = [ex for ex in new_test if ex["ata_id"] not in existing_ids]

        logger.info(f"{municipality_name}: {len(new_train_filtered)} new train, "
                    f"{len(new_val_filtered)} new val, "
                    f"{len(new_test_filtered)} new test examples to add.")

        # -------- MERGE (but do NOT overwrite originals) --------
        merged_train = existing_train + new_train_filtered
        merged_val = existing_val + new_val_filtered
        merged_test = existing_test + new_test_filtered

        # -------- Save merged outputs --------
        if merged_train:
            merged_train_path = os.path.join(train_dir, f"{municipality_clean}_train_merged.jsonl")
            save_jsonl(merged_train_path, merged_train)
            logger.info(f"Saved merged train ({len(merged_train)}) → {merged_train_path}")

        if merged_val:
            merged_val_path = os.path.join(val_dir, f"{municipality_clean}_val_merged.jsonl")
            save_jsonl(merged_val_path, merged_val)
            logger.info(f"Saved merged val ({len(merged_val)}) → {merged_val_path}")

        if merged_test:
            merged_test_path = os.path.join(test_dir, f"{municipality_clean}_test_merged.jsonl")
            save_jsonl(merged_test_path, merged_test)
            logger.info(f"Saved merged test ({len(merged_test)}) → {merged_test_path}")

        # -------- Add to combined accumulators --------
        combined_train.extend(merged_train)
        combined_val.extend(merged_val)
        combined_test.extend(merged_test)

    # ------------------------------------------------------------------
    # Save COMBINED merged datasets
    # ------------------------------------------------------------------
    save_jsonl(os.path.join(args.output_dir, "combined_train_merged.jsonl"), combined_train)
    save_jsonl(os.path.join(args.output_dir, "combined_val_merged.jsonl"), combined_val)
    save_jsonl(os.path.join(args.output_dir, "combined_test_merged.jsonl"), combined_test)

    logger.info("\n=== Merge completed successfully ===")
    logger.info(f"Total merged train: {len(combined_train)}")
    logger.info(f"Total merged val:   {len(combined_val)}")
    logger.info(f"Total merged test:  {len(combined_test)}")
