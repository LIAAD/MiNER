...
import json
import os
from transformers import BertTokenizerFast, XLMRobertaTokenizerFast, AutoTokenizer, AutoModelForMaskedLM
...
base_input = "data/metadata_paper_pt"
base_output = "data/output_paper_pt"
...
for m in municipalities:
    ...
    # --- Load JSONL splits ---
    with open(f"{base_input}/train/{m}_train_merged.jsonl", encoding="utf-8") as f:
        train_raw = [json.loads(line) for line in f]
    with open(f"{base_input}/test/{m}_test_merged.jsonl", encoding="utf-8") as f:
        test_raw = [json.loads(line) for line in f]
    with open(f"{base_input}/val/{m}_val_merged.jsonl", encoding="utf-8") as f:
    ...
    for split_name, raw_data in zip(["train", "test", "val"], [train_raw, test_raw, val_raw]):
        ...
        if split_name == "train":
            ex = examples[0]
            with open("data/output_paper_pt/residual.txt", "a", encoding="utf-8") as fout:
                fout.write(f"\n\n===== TOKENIZED EXAMPLE OF {m.upper()} =====\n")

                fout.write(f"input_ids (len): {len(ex['input_ids'])}\n")
                fout.write(f"labels (len): {len(ex['labels'])}\n\n")

                all_tokens = tokenizer.convert_ids_to_tokens(ex["input_ids"])
                all_labels = ex["labels"]

                fout.write("TOKEN\tLABEL\n")
                for tok, lbl in zip(all_tokens, all_labels):
